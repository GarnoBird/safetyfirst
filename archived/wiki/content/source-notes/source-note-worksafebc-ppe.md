---
id: "source-note-worksafebc-ppe"
title: "Personal protective equipment"
publisher: "WorkSafeBC"
sourceType: "PPE guidance"
url: "https://www.worksafebc.com/en/health-safety/tools-machinery-equipment/personal-protective-equipment-ppe"
jurisdiction: "BC"
lastChecked: "2026-06-20"
sourceIds:
  - "worksafebc-ppe"
regulationRefs:

citations:
  - "worksafebc-ppe"
relatedArticles:
  - "eye-and-face-protection"
  - "gloves"
  - "hard-hats"
  - "high-visibility-apparel"
  - "ppe-basics"
  - "respirators"
  - "safety-footwear"
---

# Personal protective equipment

## Source scope

Personal protective equipment is a public ppe guidance source from WorkSafeBC. The wiki uses it as supporting context only according to the source hierarchy and does not treat guidance or industry resources as legislation unless an official legal source supports that claim.

## Plain-language review notes

- WorkSafeBC PPE hub for eye and face protection, safety footwear, gloves, headgear, high-visibility clothing, and related PPE.
- Use this source to strengthen plain-language explanations, practical controls, or reviewer context without copying manual text.
- Legal requirement bullets should still cite exact OHS Regulation, Act, or Code references where a legal claim is made.
- If this source changes, confirm whether affected article summaries, checklists, forms, or related field tools need updates.

## Review handling

- Last checked: 2026-06-20.
- Use this note to support article review, not as a substitute for the official source.
- Keep article text original and paraphrased. Do not copy manuals, proprietary procedures, or long source passages.

## Related article review targets

- [[eye-and-face-protection]]
- [[gloves]]
- [[hard-hats]]
- [[high-visibility-apparel]]
- [[ppe-basics]]
- [[respirators]]
- [[safety-footwear]]
